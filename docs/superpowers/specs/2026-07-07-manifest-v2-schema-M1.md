...
* **`If-None-Match`:** engine передаёт `If-None-Match: <ETag предыдущего ответа>` (значение —
  `"sha256:..."` в кавычках) на `GET .../effective` (и алиас `GET .../active`) и `GET .../{manifestId}`;
  сервер сравнивает нормализованное значение (снимает `W/` и кавычки) с текущим checksum и отвечает
  `304 Not Modified` с пустым телом и тем же `ETag`, если манифест не изменился. Заголовок должен
  содержать ровно один точный ETag-токен — `*` и списки через запятую не поддерживаются (сравнение
  строковое, без wildcard-семантики). `GET .../scheduled` возвращает список дескрипторов (нет единого
  манифеста → нет единого checksum для ETag) и `If-None-Match` **не поддерживает**.
